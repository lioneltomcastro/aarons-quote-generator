...
st.set_page_config(page_title="Aaron's Quote Generator", layout="wide")
...
def payment_text(deposit):
    if deposit == "100%":
        return "Payment Terms: Full payment upfront is required prior to commencement of works."
    if deposit == "Payment upon completion":
        return "Payment Terms: Payment is to be made upon completion of works, unless negotiated otherwise at the time of contract."
    if deposit == "0%":
        return "Payment Terms: No upfront deposit is required. The balance is to be paid upon completion of works, unless negotiated otherwise."
    return f"Payment Terms: Strictly {deposit} upfront deposit is required to secure and schedule the booking. The remaining balance is to be paid upon completion of works, unless negotiated otherwise at the time of contract."
...
def generate_pdf():
    ...
    elements.append(Paragraph(f"<b>Quote:</b> {quote_address}", normal))
    ...
    elements.append(Paragraph(f"<b>Quotation:</b> {job_title}", normal))
    elements.append(Paragraph(f"<b>Location:</b> {quote_address}", normal))
    ...
    if scope1.strip():
        pricing_rows.append(["Scope 1", Paragraph(scope1, table_text), Paragraph(price1, table_text)])
    if scope2.strip():
        pricing_rows.append(["Scope 2", Paragraph(scope2, table_text), Paragraph(price2, table_text)])
    if scope3.strip():
        pricing_rows.append(["Scope 3", Paragraph(scope3, table_text), Paragraph(price3, table_text)])
    if scope4.strip():
        pricing_rows.append(["Scope 4", Paragraph(scope4, table_text), Paragraph(price4, table_text)])
    ...
    pricing_table = Table(table_data, colWidths=[65, 300, 90], repeatRows=1)
    ...
    pricing_block.append(Spacer(1, 12))

    validity_text = f"This quotation is valid for {quote_validity} from the date issued, after which it will be subject to confirmation or review."

    pricing_block.append(Paragraph("The quote does not include the following unless specifically stated in the above paragraphs:", normal))
    pricing_block.append(Paragraph(f"• {validity_text}", normal))
    pricing_block.append(Paragraph(f"• {payment_text(deposit_required)}", normal))
...
if st.button("Generate Formal PDF Preview"):
    ...
    file_name = f"Quote - {quote_address}.pdf".replace("/", "-")
